...
// $Id: functions.php,v 1.6 2004/12/11 04:38:10 nobu Exp $
...
function strim($s, $l = 50)
{
    ...
    return htmlspecialchars(mysubstr($s, 0, $h) . '...' . mysubstr($s, $t), ENT_QUOTES | ENT_HTML5);
}
...
function make_page_index($title, $max, $cur, $format, $asis = ' <b>[%d]</b>')
{
    ...
    $npg = (int)(($max - 1) / $trackConfig['list_max']) + 1;
...
function make_track_item($data, $add = '', $attr = "target='_blank'")
{
    ...
    return "<a href='$url'$alt $attr class='trtitle'>$title</a>$add" . "<div class='trtext'>$ctext</div>" . "<div class='trinfo'>" . _TB_REF_COUNT . ":$nref [" . _TB_REF_CDATE . " $cdate] [" . _TB_REF_MDATE . " $mdate]<br>" . _TB_REF_URL . " <a href='$url'>" . myurldecode($url) . '</a></div>';
}

$tags = preg_match('/^XOOPS 1\\./', XOOPS_VERSION) ? ['bg1', 'bg3'] : ['even', 'odd'];
